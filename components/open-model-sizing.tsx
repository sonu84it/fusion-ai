...
function memoryPercent(value: number, ceiling: number) {
  return Math.max(4, logScale(value, 0, 100, ceiling));
}
...
export function OpenModelSizing() {
  ...
  const mobileMemoryCeiling = Math.max(...specs.map((spec) => storageGb(spec.parametersB, 16)));
    ...
    <section id="sizing" className="relative z-10 mx-auto max-w-7xl scroll-mt-8 px-5 pb-24 lg:px-8" aria-labelledby="sizing-title">
      <div className="rounded-[30px] border border-white/[.1] bg-[#0b1124]/76 p-5 shadow-2xl sm:p-8">
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-2">
          <section className="min-w-0 rounded-2xl border border-white/[.08] bg-[#080e20]/65 p-4 sm:p-5" aria-labelledby="storage-title">
          ...
          <section className="min-w-0 rounded-2xl border border-white/[.08] bg-[#080e20]/65 p-4 sm:p-5" aria-labelledby="memory-title">
            ...
            <div className="mt-5 space-y-3 lg:hidden" role="img" aria-label="Compact mobile chart comparing estimated INT4 inference memory and mixed-precision Adam training memory"><p className="text-[11px] leading-5 text-slate-500">Log-scaled bars preserve the relative range while keeping every model and value visible on narrow screens.</p>{specs.map((spec) => { const inference = storageGb(spec.parametersB, 0.5); const training = storageGb(spec.parametersB, 16); return <div key={spec.name} className="rounded-xl border border-white/[.06] bg-white/[.02] p-2.5"><div className="mb-2 flex items-baseline justify-between gap-3"><span className="min-w-0 truncate text-xs font-medium text-slate-200">{bubbleLabels[spec.name] ?? spec.name}</span><span className="shrink-0 text-[10px] text-slate-500">{parameterLabel(spec)}</span></div><div className="grid grid-cols-[2.5rem_minmax(0,1fr)_3.5rem] items-center gap-2 text-[10px]"><span className="font-semibold text-cyan-200">INT4</span><div className="h-1.5 overflow-hidden rounded-full bg-cyan-400/10"><div className="h-full rounded-full bg-cyan-400" style={{ width: `${memoryPercent(inference, mobileMemoryCeiling)}%` }}/></div><span className="text-right text-slate-400">{formatGb(inference)}</span><span className="font-semibold text-pink-200">Train</span><div className="h-1.5 overflow-hidden rounded-full bg-pink-400/10"><div className="h-full rounded-full bg-pink-400" style={{ width: `${memoryPercent(training, mobileMemoryCeiling)}%` }}/></div><span className="text-right text-slate-400">{formatGb(training)}</span></div></div>; })}</div>
            <div className="mt-5 hidden lg:block"><svg viewBox={`0 0 ${width} ${memoryHeight}`} className="block w-full" role="img" aria-label="Bar chart comparing estimated INT4 inference memory and mixed precision Adam training memory"><title>Inference and training memory estimates</title><desc>Inference uses INT4 weights only. Training estimate is a rule of thumb that includes weights, gradients, master weights, and optimizer state. MoE bars use total weights.</desc>{memoryTicks.map((tick) => { const x = logScale(tick, left, right, 100000); return <g key={tick}><line x1={x} x2={x} y1="34" y2={memoryHeight - 28} stroke="rgba(148,163,184,.14)"/><text x={x} y="20" textAnchor="middle" fill="#94a3b8" fontSize="12">{tick >= 1000 ? `${tick / 1000} TB` : `${tick} GB`}</text></g>; })}{specs.map((spec, index) => { const y = top + index * 34; const inference = storageGb(spec.parametersB, 0.5); const training = storageGb(spec.parametersB, 16); const inferenceEnd = logScale(inference, left, right, 100000); const trainingEnd = logScale(training, left, right, 100000); return <g key={spec.name}><text x={left - 12} y={y + 17} textAnchor="end" fill="#cbd5e1" fontSize="13">{spec.name}</text><rect x={left} y={y} width={Math.max(2, inferenceEnd - left)} height="9" rx="4.5" fill="#06b6d4"><title>{`${spec.name} (${parameterLabel(spec)}): INT4 inference weights ≈ ${formatGb(inference)}`}</title></rect><rect x={left} y={y + 12} width={Math.max(2, trainingEnd - left)} height="9" rx="4.5" fill="#ec4899"><title>{`${spec.name} (${parameterLabel(spec)}): mixed-precision Adam training estimate ≈ ${formatGb(training)}`}</title></rect></g>; })}</svg></div>
